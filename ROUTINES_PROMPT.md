...
2. `raw-items.json` を読み、以下のいずれかに該当する記事を**優先的に**選ぶ:
   ...
   - **overseas カテゴリの条件付き例外採用（race_week モード）**: 通常は overseas を除外するが、
     以下の**両条件**を同時に満たす記事は国内記事の優先枠を埋めた後の余白に限り **1日最大2件** まで採用可。
     - 条件A: タイトルに下記の主要海外G1レース名のいずれかを含む:
       「凱旋門賞」「ブリーダーズカップ」「キングジョージ」「英ダービー」「仏ダービー」
       「英チャンピオンズS」「ドバイワールドカップ」「ドバイシーマクラシック」「香港カップ」「香港マイル」
     - 条件B: タイトルまたは本文に日本馬・日本産種牡馬の産駒の名前（ローマ字表記）が含まれる
     - **計測記録**: 採用した場合は ops-log の「カテゴリ別採用状況」に「overseas 例外採用: X件」と明記し、
       採用できなかった記事タイトルも1件記録する。次の主要海外G1（前後2週間）での件数推移を追う。
3. 選んだ記事の本文を `npm run --silent fetch-article` で取得し、要約する。
   - **本文取得失敗時のフォールバック**: `fetch-article` で取得した本文が 200 字未満の場合、
     そのサイトは JS 動的レンダリングのため本文が取れない状態と判断する。
     `raw-items.json` の `description` フィールド（RSS から取得した冒頭文、通常 300〜600 字）を
     本文代替として使用して要約する。
     - netkeiba.com（`pid=news_view` 形式 URL）は現時点でこの状態が続いている（JS 依存）。
     - Yahoo!ニュース経由の記事（東スポ競馬・馬トク報知・SPAIA・競馬ラボ・競馬のおはなし）は
       1000〜1800 字の本文取得が可能なため、同じ話題の記事が raw-items.json にあれば
       そちらを優先的に選択してよい。
   要約時に必ず以下を抽出:
