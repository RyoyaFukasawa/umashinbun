...
#### overseas カテゴリの英語記事の扱い

`category: "overseas"` の英語記事は、以下のルールで **日本語要約だけ書く**（英語原文の訳出は不要）:

- `title_ja` に日本語タイトルを生成する（英語タイトルをそのまま使わない）。
- `summary` は全文日本語で書く（原文が英語でも翻訳して要約する）。
  固有名詞・略称・引用符内の原文表記は原語のまま保持してよい。
- レース名は日本語通称を優先する（Arc → 凱旋門賞、BC Classic → BCクラシック、
  Hong Kong Cup → 香港カップ、King George → キングジョージ 等）。
- `horses` / `sires` の馬名はカタカナに直す（Calandagan → カランダガン 等）。
  日本での通称が定まっていない場合は英語名のままでよいが、summary 内に括弧で
  カタカナを添える（例: Calandagan（カランダガン））。
