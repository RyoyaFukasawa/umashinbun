...
1. `raw-items.json` から、以下を中心に **最大5件**選ぶ:
   ...
   **pog カテゴリの海外G1記事・業界ニュース記事の再分類（2026-06-28 時点の判断基準メモ）:**  
   > 以下は確定的なルールではなく、現時点での判断基準の記録です。実運用では文脈に応じて調整してください。
   - 「競馬のおはなし」等の `pog` カテゴリフィードが、大型海外G1を主題とする記事を配信することがある（凱旋門賞・アイリッシュダービー・BCクラシック・BCターフ・キングジョージVI&QES・ドバイワールドカップ等）。この場合、実質的なカテゴリは `overseas` であるため、`"category": "overseas"` として採用を優先する。（レース名リストは例示。随時更新を推奨する。）
   - 同様に、pog カテゴリ記事でも実質が業界ニュース（騎手の海外短期免許取得・女性騎手マイルストーン等）の場合は `"category": "news"` に再分類してよい。
   - 採用した場合は ops-log の「カテゴリ別採用状況」に `overseas: X（元 pog から再分類）` または `news: X（元 pog から再分類）` と記録する。
